...
# One to three steps, depending on whether this host has a Thai/Arabic font.
...
page = Path('output/web/lever.html').read_text(encoding='utf-8')
assert 'TutorDrawPlayer' in page and 'data-drawcv-raster' not in page, 'web page'
...
hops = sorted(Path('output/number_line').glob('step-*.png'))
...
# Fail loudly rather than silently passing: a green matrix must prove that at
# least one candidate codec really encoded, not merely that nothing crashed.
videos = sorted(Path('output/video').glob('cell-lesson.*'))
assert videos, ('No video was produced. Either this host provides none of the '
                'candidate codecs, in which case record that and relax this check '
                'deliberately, or video export regressed.')
for path in videos:
    ...
    try:
        ...
        capture.release()
print(f'Verified {len(paths)} PNGs, {len(videos)} video(s), and reloaded lessons outside the checkout')
"""
