...
def test_timing_roundtrip_and_v1_upgrade(lesson):
    ...
    assert upgraded.to_dict()["schema_version"] == 2
...
def test_timing_fields_required_only_in_v2(lesson):
    data = lesson.to_dict()
    ...
    with pytest.raises(LessonFormatError, match="unknown"):
        Tutorial.from_dict(data)
